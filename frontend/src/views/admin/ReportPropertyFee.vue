<template>
  <div class="page">
    ...
    <template v-if="!loadError">
      ...
      <el-row :gutter="16" style="margin-bottom:16px">
        <el-col :span="8">
          <el-tag type="success" size="large">已缴：{{ data.paidCount }} 户</el-tag>
        </el-col>
        <el-col :span="8">
          <el-tag type="warning" size="large">待缴：{{ data.pendingCount }} 户</el-tag>
        </el-col>
        <el-col :span="8">
          <el-tag type="danger" size="large">逾期：{{ data.overdueCount }} 户</el-tag>
        </el-col>
      </el-row>
      ...
      <el-card>
        <div class="page-card-header">
          ...
          <span class="page-card-total">共 {{ data.details ? data.details.length : 0 }} 户</span>
        </div>
...
const getSummaries = (param) => {
  ...
  columns.forEach((col, index) => {
    ...
    if (col.property === 'amount') {
      sums[index] = '¥' + fmt(data.totalReceivable)
    } else if (col.property === 'collectedAmount') {
      sums[index] = '¥' + fmt(data.totalCollected)
...
const loadData = async () => {
  ...
  try {
    ...
    const res = await request2.get('/property-fee/summary', { params })
